...
def call_func_sc_confirm_trx(adr_of_owner):
    ...
    f = contract.functions.confirmTransaction(1)
...
def call_function_from_sc_exec_transaction():
    ...
    abi = ms_sc_abi
    contract = web3.eth.contract(address=web3.to_checksum_address(address_contract), abi=abi)
    print(contract.functions.getTransaction(1).call())
    ...
    f = contract.functions.executeTransaction(1)
    unsigned_tx = f.build_transaction(build_trx_config)
...
def load_contract():
    ...
    contract_id, contract_interface = compiled_sc.popitem()
    # get abi
    ...
    ms_sc = web3.eth.contract(abi=abi, bytecode=bytecode)
    ...
    gas = web3.eth.estimate_gas(build_trx_config) + 2000000
    build_trx_config['gas'] = gas

    called_constructor = ms_sc.constructor([first_adr, second_adr], 2)
...
def make_transaction():
    ...
    tx_dict = {
        'chainId': web3.eth.chain_id,
        'from': web3.to_checksum_address(first_adr),
        'to': web3.to_checksum_address(goerli_ms_sc_adr),
        'nonce': web3.eth.get_transaction_count(first_adr),
